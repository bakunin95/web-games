...
Gauntlet Playground — atmospheric Canvas 2D effects with live parameter tweaking. See `README.md` for run instructions, effect API, and license policy.

## Cursor Cloud specific instructions

### Services
- **Gauntlet Playground (Vite)** — only app in this repo. Start with `pnpm dev` (binds `0.0.0.0:5173`). Production check: `pnpm build` then `pnpm preview` on port `4173`.
...
- **Flagship gauntlet demo**: Building Window (Rain→Shatter) in Create VFX — rain drops on glass that shatters on click. Place fire/neon behind window to see compositing through wet glass.
