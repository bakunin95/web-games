...
function makeLobes(rand: () => number, count: number): Lobe[] {
  ...
  for (let i = 0; i < count; i++) {
    const tier = i / count; // early = larger primaries
    const primary = tier < 0.3;
    const ang = rand() * Math.PI * 2;
    const rad = primary ? 0.15 + rand() * 0.4 : 0.4 + rand() * 1.05;
    // Wide scale range — critical for cauliflower, not uniform balls.
...
/** Fill an irregular cauliflower lobe as 1–3 overlapping ellipses (not a perfect ball). */
function fillIrregularLobe(
  ...
  ctx.fill();
  // Satellite bumps break the sphere silhouette.
  ctx.beginPath();
  ctx.ellipse(
    lx + L.bumpAx * lrx,
    ly + L.bumpAy * lry,
...
function drawLobe(
  ...
  // Pass 1 — dark / mid mass (baseColor). Continuous volume, not a glow halo.
  ...
  // Pass 2 — underside + crevice self-shadow (darker baseColor).
  ...
  // Pass 3 — soft top/windward wash (emissive). Crescent, not full-ball highlight.
  ...
  if (litA > 0.02) {
    ...
    ctx.fill();
    // Secondary bump highlight for irregular lit rim.
    if (L.litBias > 0.45) {
...
export const drawSmoke: DrawFn<SmokeParams> = (ctx, params, t, scene) => {
  ...
  for (const p of sorted) {
    ...
    // Inter-lobe valley: darken cluster guts before stacking lit rims.
    ...
    // Ragged dissipation: thin filaments + flecks along wind — not a sealed haze.
    if (k > 0.12) {
      ...
      for (let w = 0; w < filaments; w++) {
        ...
        ctx.fillStyle = wg;
        ctx.beginPath();
        // Elongate with wind for streaky wisps.
